...
import { useRemoveUserMutation } from '../../../features/slices/users/usersSlice'
...
import { useGetAuditListQuery } from '../../../features/slices/auditlog/auditlogSlice'
...
const Users = () => {
  ...
  const { data, isFetching } = useGetAuditListQuery({
    page,
  ...
  const columns = [
    ...
    },
    {
      ...
      dataIndex: 'ApplicationModuleName',
      // sorter: (a, b) => a.role.localeCompare(b.role),
    },
    {
    ...
    <DefaultLayout>
      <div className={classes.Users}>
        <section className={classes.Users__headerSection}>
          <PageBreadcrumb title="Audit Logs" items={['Audit Logs']} />
        </section>
        <TableWithFilter
          columns={columns}
          data={data?.results}
      ...
      <Suspense fallback={<h4>Loading...</h4>}></Suspense>
      <ToastContainer />
    </DefaultLayout>
  )
}
